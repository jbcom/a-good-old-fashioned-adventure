...
- [ ] S9.6 Class economy realignment (user mandate 2026-06-11, precedes S9.4): class unlock nodes cost roses, not coins; every class gains its own coin-funded upgrade track as a DAG branch off its class node; rename the `upgradeWeb` config key and UI surfaces to the DAG framing in the same commit (refactor with all callers, no shim); update `src/config/incremental.json`, doc language, and the rarity/cost unit tests together
