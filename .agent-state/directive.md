...
- 2026-06-11 S7.3 pause-test correction: browser pause assertions must capture position after the pause UI/state has settled, prove movement is frozen during pause, then prove movement resumes after unpause. Comparing against a pre-pause frame can fail on legitimate frame-boundary drift.
- 2026-06-11 S7.3 pause correction: pause must synchronously clear directional input before React state commits, and paused direction keydowns must not arm movement for the next frame. Keep the browser pause test strict; a few pixels of drift is a real control bug.
