# State

Updated: 2026-06-11

Branch: `feat/content-architecture`

## Current Build State

- Vite/React/TypeScript app scaffolded.
- Content-first JSON registry, schemas, Koota sim, Yuka enemy behaviors, ToneJS
  audio, AnimeJS motion, r3f HD-2D renderer, and storybook UI are in place.
- Expanded route: Hearthwake Village -> Oldwood Forest -> Deep Forest ->
  Sunken Road -> Castle Approach -> Castle Dungeon -> Victory.
- Browser playthrough validates the expanded journey through public controls.
- Capacitor Android scaffold, SQLite/Preferences/Device plugins, web SQLite
  wasm copy, `cap:sync`, and debug APK assemble are green locally.

## Active Queue

See `.agent-state/directive.md`. S7.2 owns CI, release-please, and standard repo
docs. S7.3 owns PR creation, green checks, and merge.
